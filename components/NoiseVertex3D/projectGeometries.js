...
export const createProjectGeometries = () => {
    const geometries = [];
    ...
    const vrMesh = new THREE.Mesh(vrGeometry, vrMaterial);
    ...
    const mrMesh1 = new THREE.Mesh(mrGeometry1, mrMaterial1);
    ...
    const mrMesh2 = new THREE.Mesh(mrGeometry2, mrMaterial2);
...
export const handleProjectInteractions = (raycaster, camera, projects, onProjectClick) => {
    raycaster.setFromCamera(new THREE.Vector2(), camera);
    const intersects = raycaster.intersectObjects(projects);

    // Reset all projects not being interacted with
    projects.forEach(project => {
        if (!intersects.find(intersect => intersect.object === project)) {
...
export const animateProjectSelection = (project, camera, onComplete) => {
